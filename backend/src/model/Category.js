...
Category.prototype.getParentCategories = async function () {
    const data = await this.db(this.tableName).where("parent_id", null)
    return data;
}
...
Category.prototype.create = async function ({
    ...
    meta_key, meta_description, no_follow = 0, no_index = 0, status = 1, parent_id
}) {
