...
const getParentCategories = async (req, res, next)=>{
    try {
        return res.status(200).send(responseBuilder(200, await category.getParentCategories(), 'Successfully fetched data'))
    } catch (e) {
        return next(e);
    }
}
const create = async (req, res, next) => {
    ...
    try {
        return res.status(200).send(responseBuilder(200, await category.create({
            title,  content_body, created_by, meta_title,
            meta_key, meta_description, no_follow, no_index, status, created_by: req.user_id, parent_id
...
const update = async (req, res, next) => {
    const {
        title,  content_body, created_by, meta_title,
        meta_key, meta_description, no_follow, no_index, status, parent_id
    } = req.body;
    const validator = await Validator([
        {
            key: 'title',
...
module.exports = { index, create, get, update, remove, getParentCategories };